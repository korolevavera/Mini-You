...
def save_user_field(user_id, field, value)
    ...
    conn.execute(fUPDATE users SET {field} =  WHERE user_id = , (value, user_id))
    conn.commit()
    conn.close()
...
def reset_user(user_id)
    ...
    conn.commit()
    conn.close()
    logging.info(fUser {user_id} reset)
...
QUICK_TEST = [
    ...
    {question В трудной ситуации тебе важнее..., option_a 🧩 Найти логичное решение, option_b ❤️ Сохранить гармонию, a_dim T, b_dim F},
    ...
    {question При принятии решения ты..., option_a 🧠 Анализируешь все за и против, option_b ❤️ Прислушиваешься к интуиции, a_dim T, b_dim F},
    {question Ты предпочитаешь..., option_a 🗓️ Иметь чёткий план на день, option_b 🔄 Импровизировать по ходу, a_dim J, b_dim P},
...
def send_action(chat_id, action='typing')
    url = fhttpsapi.telegram.orgbot{TOKEN}sendChatAction
...
def send_message(chat_id, text, parse_mode='Markdown', retries=3)
    send_action(chat_id)
    url = fhttpsapi.telegram.orgbot{TOKEN}sendMessage
    payload = {'chat_id' chat_id, 'text' text, 'parse_mode' parse_mode}
    for attempt in range(retries)
        try
            r = requests.post(url, json=payload, timeout=10)
            logging.info(fСообщение отправлено (попытка {attempt+1}), статус {r.status_code})
            return r.json()
        except Exception as e
            logging.error(fОшибка отправки (попытка {attempt+1}) {e})
            time.sleep(1)
    logging.error(fНе удалось отправить сообщение после {retries} попыток)
    return None
...
def send_keyboard(chat_id, text, keyboard, parse_mode='Markdown')
    ...
    try
        ...
        logging.error(fОшибка отправки клавиатуры {e})
        return None
...
def show_test_question(chat_id, user_id, index)
    q = QUICK_TEST[index]
    text = f🧠 Вопрос {index+1} из 12nn{q['question']}
    buttons = [
        [fА {q['option_a']}],
        [fБ {q['option_b']}]
    ]
...
def webhook()
    ...
    try
        ...
        logging.info(fПолучен запрос {data})
...
def handle_message(chat_id, user_id, user, text)
    ...
    if text == 'reset_me'
        reset_user(user_id)
        send_message(chat_id, 🔄 Аккаунт полностью сброшен! Теперь ты как новый пользователь.nnНапиши start, чтобы начать с приветствия.)
        return

    if status == 'not_started'
        if text == 'start'
            welcome_text = (
                👋 Привет! Я — твой Мини-Ты.n
                Я здесь, чтобы помочь тебе лучше понять себя.n
                Вместе мы пройдём путь, который поможет тебе увидеть свои сильные стороны, найти опору и услышать то, что внутри.nn
                Вот что мы с тобой сделаемnn
                🧠 Узнаем твой архетип — твою суперсилу и зону роста.n
                📋 Составим расписание с практиками и аффирмациями на каждый день.n
                🚪 Откроем Комнату — 7 дней вопросов к себе, которые помогут тебе стать ближе к себе.n
                📊 Будем смотреть прогресс — чтобы тебе было видно, сколько уже пройдено.nn
                Готов(а) заглянуть внутрь себяn
                Нажми кнопку 👇
            )
            keyboard = {
                'inline_keyboard' [
        ...
        name = text.strip()
        if len(name) = 2 and len(name) = 20 and not text.startswith('') and re.match(r'^[a-zA-Zа-яА-ЯёЁ0-9s-]+$', name)
            ...
            welcome_text = (
                f✅ Имя {name} сохранено!nn
                fОтлично! Теперь у тебя есть спутник — {name}.nn
                Я — твой помощник по самонаблюдению. Вот что я умеюnn
                🧠 Мой Архетип — узнать свою суперсилу и зону роста.n
                📋 Расписание — практики и аффирмации на день.n
                🚪 Комната — 7 дней вопросов к себе.n
                📊 Прогресс — смотреть, сколько пройдено.nn
                Выбирай, с чего начнём 👇nn
    ...
    if text.startswith('')
        if text == 'start'
            if status == 'testing'
                ...
                return
            show_main_menu(chat_id, fГлавное меню, {name})
        elif text == 'test'
            if status == 'testing'
    ...
    if text == 🏠 Главная
        ...
        if user.get('archetype')
            show_submenu(chat_id, 
                f📋 Расписание для архетипа «{ARCHETYPE_NAMES.get(user['archetype'], '')}»nn
                🌅 Утро Практика и аффирмацияn
                ☀️ День Практика и аффирмацияn
                🌙 Вечер Практика и аффирмацияnn
        ...
        send_message(chat_id, 
            🚪 Комнатаnn
            Это твоё личное пространство для самонаблюдения.n
            Здесь ты будешь проходить 7 дней вопросов к себе.nn
        ...
        show_submenu(chat_id, 
            📊 Твой прогрессnn
            f🧠 Архетип {ARCHETYPE_NAMES.get(user.get('archetype'), 'не определён')}n
            🚪 Комната 0 из 7 днейn
            📋 Расписание не настроеноnn
        ...
        show_submenu(chat_id, 
            ⚙️ Настройкиnn
            Здесь можно настроить напоминания и управлять данными.nn
            _(Функция появится в следующем обновлении!)_
        )
        ...
        send_message(chat_id, 
            fТы уже зарегистрирован(а) как {name}.nn
...
def finish_test(chat_id, user_id)
    ...
    send_message(chat_id,
        f🧠 Твой архетип — {archetype_name}nn
        f{description}nn
        Теперь ты можешьn
        📋 Посмотреть расписание для своего архетипаn
        🚪 Войти в Комнату и начать прокачкуn
        📊 Отслеживать прогрессnn
...
# =============================================
# ЗАПУСК (для Gunicorn используется переменная PORT)
# =============================================
