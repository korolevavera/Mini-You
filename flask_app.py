...
def calculate_archetype_from_params(params, base_archetype=None):
    # Сопоставление параметров с MBTI (упрощённо)
    # Используем существующий архетип, если он уже есть, или вычисляем новый
    if base_archetype:
        # Если уже есть, можно уточнить по параметрам (пока оставляем как есть)
        return base_archetype
    # Простейшая логика: считаем сумму параметров, отнесём к одному из 16
    # Для демо возьмём ENFP, если много imagination, и т.д.
    # В реальности можно сделать более сложную систему.
    # Пока вернём ENFP как пример
    return "ENFP"
...
ARCHETYPE_DESC = {
    ...
    # ... все остальные архетипы (я сократил для экономии места, но в полном коде они есть)
    "ENTJ": ("Ты — Командир...",),
    "INTP": ("Ты — Мыслитель...",),
    "ENTP": ("Ты — Новатор...",),
    "INFJ": ("Ты — Наставник...",),
...
# =============================================
# НАСТРОЙКИ РАСПИСАНИЯ (без изменений)
# =============================================
...
# =============================================
# НОВАЯ ЛОГИКА КОМНАТЫ (НОВЕЛЛА)
# =============================================
def handle_room(chat_id, user_id, user):
    # Проверяем, есть ли состояние новеллы
    state = get_novel_state(user_id)
    if state is None:
        state = create_novel_state(user_id)
    
    # Если новелла завершена
    if state.get('completed', 0) == 1:
        # Показываем финал и предложение продолжения
        room_text = build_final_room(user, state)
        send_message(chat_id, room_text)
        # Предложение купить продолжение (заглушка)
        keyboard = {
            'inline_keyboard': [
    ...
    # Если есть сохранённый эпизод (episode > 0), продолжаем
    episode_id = state.get('episode', 0) + 1
    if episode_id > 7:
        # Все эпизоды пройдены, отмечаем как завершённые
        state['completed'] = 1
        save_novel_state(user_id, state)
...
def handle_novel_choice(chat_id, user_id, episode_id, choice_idx):
    ...
    # Показываем нейтральный текст эпизода и переходим к следующему
    send_message(chat_id, f"🌀 *Результат*\n\n{ep['neutral']}")
    # Задержка (небольшая) перед следующим эпизодом
    time.sleep(0.5)
    # Показываем следующий эпизод или финал
...
def build_final_room(user, state):
    # Формируем портрет на основе параметров
    params = state.get('parameters', {})
    # Определяем архетип (если есть, используем существующий)
    archetype = user.get('archetype')
    if not archetype:
        archetype = calculate_archetype_from_params(params, None)
        # Можно сохранить в базу
        save_user_field(user['user_id'], 'archetype', archetype)
    archetype_name = ARCHETYPE_NAMES.get(archetype, "Человек")
    description = ARCHETYPE_DESC.get(archetype, "Твой уникальный архетип.")
    
    # Собираем ключевые фразы (можно использовать из game_answers)
    ...
    try:
        ...
        if data and 'callback_query' in data:
            ...
            if callback_data == 'start_game':
                ...
                return 'ok', 200
            # Обработка других callback (если есть)
...
def handle_message(chat_id, user_id, user, text):
    ...
    # Фикс: если статус not_started, но имя уже есть
    ...
    if status == 'not_started':
        ...
        if len(name) >= 2 and len(name) <= 20 and not text.startswith('/') and re.match(r'^[a-zA-Zа-яА-ЯёЁ0-9\s\-]+$', name):
            ...
            welcome_text = (
                ...
                "🚪 **Комната** — 7 эпизодов с выбором, которые изменят тебя.\n"
    ...
    if text.startswith('/'):
        if text == '/start':
            ...
            # Если пользователь уже в процессе новеллы, продолжаем
            state = get_novel_state(user_id)
            if state:
                handle_room(chat_id, user_id, user)
            else:
                # Если нет, но есть старые записи (game_day), предложим начать новеллу заново
                if user.get('game_day', 0) > 0:
                    send_message(chat_id, "У тебя есть незавершённая старая Комната. Хочешь начать новую историю? (все старые данные будут сохранены в истории)")
    ...
    if text == "🧠 Мой Архетип":
        if user.get('archetype'):
            ...
            send_message(chat_id, "🧠 Ты ещё не проходил(а) тест.\nНажми /test, чтобы узнать свой архетип.")
        return
    if text == "📋 Расписание":
        archetype = user.get('archetype')
        if archetype:
    ...
    if text == "🚪 Комната":
        ...
        # Проверяем, есть ли прогресс новеллы
        state = get_novel_state(user_id)
        if state:
            handle_room(chat_id, user_id, user)
            return
        else:
            # Если нет, но есть старые записи (game_day), можно предложить начать новую
            if user.get('game_day', 0) > 0:
                send_message(chat_id, "У тебя есть незавершённая старая Комната. Хочешь начать новую историю? (все старые данные будут сохранены в истории)")
                # Показываем кнопки
                buttons = [
                    [InlineKeyboardButton("🚪 Начать новую историю", callback_data="start_new_novel")],
                    [InlineKeyboardButton("📖 Показать старую комнату", callback_data="show_old_room")]
                ]
                keyboard = InlineKeyboardMarkup(buttons)
                send_keyboard(chat_id, "Выбери действие:", keyboard)
                return
            else:
                # Создаём новое состояние
                create_novel_state(user_id)
                handle_room(chat_id, user_id, user)
        return
    ...
    # Обработка игры (активный статус) – пока оставляем старую логику для совместимости
    if status == 'active':
    ...
    # Неизвестный текст – возможно, отчёт
    # Обрабатываем как отчёт (сохраняем в key_phrases для истории)
    if text:
